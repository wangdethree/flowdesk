...
7. 启动 `frontend/`，通过 Vue 管理后台完成登录、看板统计、工单流转、智能助手、标签管理和通知筛选。
...
| POST | `/api/ai/ticket-draft/` | 根据自然语言描述生成工单草稿建议 |
| GET | `/api/ai/tickets/{id}/suggestion/` | 查询已有工单的智能处理建议 |
...
- 智能助手接口复用当前用户可见工单范围，不会让普通用户读取无权工单。
- 工单草稿建议当前使用离线规则引擎，后续可以替换为大模型或 Agent 编排。
...
- 分页体验：工单和通知列表使用后端分页，前端展示总数和上一页/下一页。
- 通知系统：业务事件触发站内通知，并支持已读、未读、筛选和清理。
- 智能助手：用独立模块封装建议生成逻辑，体现后续接入大模型的扩展点。
